...
function switchQuestion(){
    window.location.replace("/maths/en/adds3/q6/index.html");
}

function calculateScore() {
    let score = (starCount * scoreFactor) / (second + minute * 60 + hour * 3600);
    score = score.toFixed(2);
    sessionStorage.setItem("adds3-q5", score);

    let newScore = Number(sessionStorage.getItem("adds3-q1"))
        + Number(sessionStorage.getItem("adds3-q2"))
        + Number(sessionStorage.getItem("adds3-q3"))
        + Number(sessionStorage.getItem("adds3-q4")) + Number(score);
