...
/* LOGIC */
function q5CheckTotalValue(id, actual) {
...
function q5CheckMissingValuesAll(qId) {
    let isCorrect = true;
    if (qId === 'q5') {
...
function switchQuestion(){
    window.location.replace("/maths/si/adds2/q4/index.html");
}

function calculateScore() {
    let score = (starCount * scoreFactor) / (second + minute * 60 + hour * 3600);
    score = score.toFixed(2);
    sessionStorage.setItem("adds2-q3", score);

    let newScore = Number(sessionStorage.getItem("adds2-q1"))
        + Number(sessionStorage.getItem("adds2-q2")) + Number(score);
