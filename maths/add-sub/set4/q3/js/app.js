...
/* LOGIC */
function q24checkTotalValue(id, actual) {
...
function q24checkMissingValuesAll() {
    let isCorrect = true;
...
function switchQuestion(){
    window.location.replace("/maths/en/adds4/q4/index.html");
}

function calculateScore() {
    let score = (starCount * scoreFactor) / (second + minute * 60 + hour * 3600);
    score = score.toFixed(2);
    sessionStorage.setItem("adds4-q3", score);

    let newScore = Number(sessionStorage.getItem("adds4-q1"))
        + Number(sessionStorage.getItem("adds4-q2")) + Number(score);
