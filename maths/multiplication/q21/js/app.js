...
function drop(event) {
    ...
    if (
        ...
        (event.target.parentElement.className === "q21-letter-container" &&
            event.target.className === "q21-answer-box-drop-div")
    ) {
...
function submitAnswerQ21() {
    ...
    for (var i = 0; i < c.length; i++) {
        if (c[i].children[0].getAttribute("canswer") != ansTrack2) {
            cansdiv = document.getElementById("q21" + ansTrack2);

            setTimeout(() => {
                console.log(c[i].children[0].innerHTML);
                if (c[i].children[0].innerHTML !== "") {
                    c[i].children[0].className = "q21-check-answer";
                } else {
                    c[i].children[0].className = "q21-check-back-answer";
                }

                cansdiv.className = "q21-answer-drop-div";
            }, 1000);

            c[i].children[0].className = "q21-red-ball";
