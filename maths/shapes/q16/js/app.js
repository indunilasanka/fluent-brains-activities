...
var q2container3 = document.getElementById("q3-shapes-container3");
var q2buttons3 = q2container3.getElementsByClassName("q3-answer-name");

for (let j = 0; j < q2buttons3.length; j++) {
    q2buttons3[j].addEventListener("click", function () {
        q2Answer3 = this.innerHTML;

        var current = document.getElementsByClassName("q3-answer-check3");
        if (current.length === 0) {
            this.className += " q3-answer-check3";
        } else {
            current[0].className = current[0].className.replace(
                " q3-answer-check3",
                " "
            );
            this.className += " q23-answer-check3";
        }
    });
}
